import glossary from "@/lib/ai/data/glossary.json";

// Render-time CORRECT-or-strip of confabulated acronym expansions, ported from
// the original gap-chat (app/src/components/chat/markdown.jsx). Apertus invents
// (often WRONG) parentheticals like "vLLM (Vector-Level Language Model)" despite
// the prompt forbidding it, so we fix them deterministically before render:
...
const isAcronymTok = (tok: string) =>
  (tok.match(/[A-Z]/g) || []).length >= 2 && tok.length <= 6;

const isAside = (s: string) =>
  /^\s*(e\.?\s?g|i\.?\s?e|see|or\b|and\b|\d|https?:|"|')/i.test(s);
