...
(Note: the intro paragraphs of this document are omitted in the website UI. Please keep this heading for authoring context. Optional annotations on a step include `@ready_by:Q1 26`, `@owners:…`, `@label-color:…`, `@gap:red`, and `@status:active|partial|gap`. Only the subtitle line is displayed in the UI. Lines prefaced by `@note` are displayed below the card. All other bullet points (`-`) are not rendered.)

The AI Potluck thesis is that the public-interest movement does not need a new model lab or a new frontend. It needs a coordinated suite of products assembled from the best open-source projects currently out there, hardened to consumer- and enterprise-grade, and put in the hands of users by the Swiss AI Summit. This roadmap names those products. Each product will then have a technical roadmap that depends on the quality of the open-source components underneath.

1. `chat` Browser-based conversational client. The product most users will judge the entire movement by. @gap:red @owners:Public AI + design partner @ready_by:Q3 26 @compare:claude.ai
...
2. `mobile-app` Native iOS and Android client. Where most consumer AI usage actually happens. @owners:Public AI + contractor @ready_by:Q4 26 @compare:Claude mobile, ChatGPT mobile
   ...
   - Reuses the same backend, same auth, same memory layer as `chat`; the build is a thin client, not a parallel product. A few open source UIs may already have ports.

3. `desktop-app` Native macOS, Windows, and Linux client. For power-users. Important for local-first features, including local/rare data use-cases. @owners:Public AI + community @ready_by:Q4 26 @compare:Claude Desktop, ChatGPT Desktop
   ...
   - Linux support is a small effort that signals seriousness to the open-source community. Closed labs ship Mac and Windows only. Worth funding?
   - Local model fallback (running Apertus 8B on-device when offline or for privacy-sensitive sessions) could be a cool differentiator. Not sure if anyone is shipping this right now.

4. `foundation-model` The flagship open-source foundation model and model family. Multi-modal, tool-use, reasoning. @gap:red @owners:Switzerland @ready_by:Q2 26 @compare:Claude Sonnet, GPT-4o
...
5. `inference-platform` Developer-facing API and portal — OpenAI-compatible, sovereign-routed, with SDKs and dashboards developers actually trust. @owners:Public AI @ready_by:Q3 26 @compare:Anthropic Console, OpenAI Platform
   - platform.publicai.co exists with 2,000 developers; the work is SLAs, Python and TS SDKs, docs, billing, and uptime dashboards.
   - Multi-model routing positioned as "public + sovereign OpenRouter". The differentiation is provenance and data governance attestations per request, not feature parity.
...
6. `compute` The pooled, multi-source compute supply that the rest of the stack runs on. For us: most likely sovereign HPC, donated cycles, and (eventually) public inference infrastructure. @owners:Finland (LUMI/CSC) + Switzerland (CSCS) + partner universities @ready_by:Q2 26 @compare:no clean analogue — closed labs own captive datacenters
   @note Compute is the core contribution governments can most easily give
   - Donation accounting, scheduling, and attestation across LUMI, CSCS, CSC, and partner clusters. Bookkeeping is actually a pretty important requirement for these public compute providers since they have their own constraints.
   ...
   - Public inference infrastructure for other HPCs (later phase) is how this product scales beyond anchor partners and becomes a genuine alternative to hyperscaler captive compute. Needs research into distributed inference.

7. `edge` A reference stack for running on-device AI. Certified runtime, model, RAG, and update path that OEMs, governments, and field deployments build on. @owners:India + Current AI + community (llama.cpp, MLX, Ollama) @ready_by:Q3 26 @compare:no consumer-facing analogue — closed labs structurally can't ship flagship models to devices they don't control
...
8. `inference-orchestrator` A distributed sovereign-inference layer that routes requests across the `compute` pool with the right model, latency, and provenance guarantees. @owners:Public AI + CSCS + CSC @ready_by:Q2 26 @compare:no clean analogue — closed labs run captive infra
...
9. `evals` Independent, reproducible evaluation suites for foundation models, agentic systems, and safety... but published as a product, not a paper. @owners:Finland (ELLIS + CSC) @ready_by:Q2 26 @compare:HELM, UK AISI Inspect, MMLU leaderboards
...
10. `skills` An open marketplace and authoring surface for reusable, composable model capabilities. @owners:Current AI + community @ready_by:Q4 26 @compare:Anthropic Skills, OpenAI GPTs
...
11. `agent` A coding-and-tasks agent built on the open stack — the high-visibility demo that proves the stack can do more than chat. @owners:Potluck (Aider, Continue.dev, OpenHands) + Current AI @ready_by:Q4 26 @compare:Claude Code, Codex, Cursor
...
12. `interviewer` An AI interviewer for structured listening at scale — eliciting input from communities, citizens, and stakeholders, then synthesizing it back. The signature public-interest product the closed labs won't build. @gap:red @owners:Current AI + Public AI CH + delibtech partners @ready_by:Q4 26 @compare:Remesh, Polis, Anthropic deliberation experiments
    @note The product that proves public-interest AI is a different category, not just a cheaper one
...
13. `trust-and-safety` The moderation, red-teaming, incident response, and data-governance product that makes the stack defensible on day one of public access. @gap:red @owners:Current AI + potluck @ready_by:Q3 26 @compare:no consumer-facing analogue — closed labs run this internally
...
14. `stack-map` The live, public visualization of the open-source AI ecosystem. Meta-product essential for orchestration and coordination across the movement. @owners:Current AI + OSO @ready_by:Q2 26 @compare:LF AI & Data Landscape, Hugging Face leaderboards
    - Build data-first on OSO infrastructure before commissioning design; a polished frontend over unvalidated data is a bad failure mode.
    - The survey (G7), the paper (Columbia), and the live visualization are three deliverables around one product.