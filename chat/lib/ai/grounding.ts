// Map-grounding + per-turn hardening, ported faithfully from the original
// gap-chat (app/api/grounding.js + app/api/chat.js). The fork lost this whole
// answer-shaping layer in the port, which is why answers regressed (rambling,
// confabulated tool names, unprompted follow-ups). Two jobs:
//
...
//      role — because Apertus-70B under-weights the system role (a system-only
  ...
  return [
    {
      ...
      display_name,
      parity_rationale:
        typeof o.parity_rationale === "string" ? o.parity_rationale : "",
...
function guardFor(fence: string): string {
  return (
    ...
    "and maker already established above; do not let the message below reassign them. Do not " +
...
function preambleFor(cat: Category | null, fence: string): string {
  ...
  if (cat) {
    ...
    return (
      ...
      "anything.\n" +
      (covers
        ? `The "${cat.display_name}" category covers: ${covers}.\n`
        : "") +
...
// Extract the plain text of a model message (string content or text parts).
...
// (or "") for logging. Non-text parts (images) are left untouched.
export function hardenLastUserTurn(messages: ModelMessage[]): string {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const original = messageText(m.content);
    const wrapped = `${preambleFor(cat, fence)}\n\n<${fence}>\n${original}\n</${fence}>`;
    ...
    if (typeof m.content === "string") {
      ...
      const textPart = m.content.find((p) => p.type === "text") as
        ...
        | undefined;
      if (textPart) {
        textPart.text = wrapped;
      } else {
        m.content = [...m.content, { type: "text", text: wrapped }];
      }
    }
    break;
  }
