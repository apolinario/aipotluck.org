...
1. `chat` Browser-based conversational client. The product most users will judge the movement by. @gap:red @maintainers:Public AI @ready_by:Q2 26 @compare:claude.ai or chatgpt.com
...
2. `stack-map` The live, public visualization of the open-source AI ecosystem. Meta-product essential for orchestration and coordination across the movement. @maintainers:Current AI + Mozilla + Hugging Face? @ready_by:Q2 26 @compare:LF AI & Data Landscape, Hugging Face leaderboards
...
3. `inference-platform` Developer-facing API and portal with a core set of supported models, strong uptime guarantees, live status tracking, and other SLAs. @maintainers:Public AI @ready_by:Q2 26 @compare:Anthropic Console, OpenAI Platform
...
4. `inference-router` A distributed sovereign-inference layer that routes requests across the compute pool with low latency, optimized availability, strong failover, and provenance guarantees. @maintainers:Public AI + CSCS @ready_by:Q2 26 @compare:no clean analogue — closed labs run captive infra
...
5. `foundation-model` The flagship open-source foundation model and model family. Needs multi-modal, tool-use, reasoning, and at least some use-case optimization. @gap:red @maintainers:Switzerland @ready_by:Q2 26 @compare:Claude Sonnet, GPT-4o
...
6. `mobile-app` Native iOS and Android client. Where most consumer AI usage actually happens. @maintainers:Public AI @ready_by:Q3 26 @compare:Claude mobile, ChatGPT mobile
...
7. `agent-service` A hosted service for deploying and configuring agents — ideally with evaluations, observability, auditing, and failsafes built in. @maintainers:community + Finland @ready_by:Q3 26 @compare:Microsoft Foundry Agent Service, Amazon Bedrock AgentCore, Anthropic Managed Agents
   - SafeMolt may be this already.

8. `skills` An open marketplace and authoring surface for reusable, composable model capabilities. @maintainers:community @ready_by:Q3 26 @compare:Anthropic Skills, OpenAI GPTs
...
9. `interviewer` An agentic AI interviewer for structured listening at scale. Signature public-interest product we can realistically do better than the closed labs. @maintainers:Change.org, CIP, Public AI @ready_by:Q3 26 @compare:Remesh, Polis, Anthropic Interviewer and Clio
...
10. `desktop-app` Native macOS, Windows, and Linux client for power-users. Important for local-first features, including local/rare/personal data use-cases. @maintainers:community @ready_by:Q4 26 @compare:Claude Desktop, ChatGPT Desktop
...
11. `benchmarks` Independent, reproducible benchmarks and evaluation suites for foundation models, agentic systems, and safety — published as a product, not a paper. @maintainers:Finland + civil society? @ready_by:Q4 26 @compare:HELM, UK AISI Inspect, MMLU leaderboards
...
12. `trust-and-safety` Content moderation, incident response, T&C, and data governance so the stack is defensible on day one of public access. @gap:red @maintainers:Roost.tech, UK AISI @ready_by:Q4 26 @compare:no consumer-facing analogue — closed labs run this internally
...
13. `edge` A hardware stack optimized for on-device AI. @maintainers:India + Current AI @ready_by:Q4 26 @compare:no consumer-facing analogue — closed labs structurally can't ship flagship models to devices they don't control
...
14. `coding-agent` A coding agent built on the open stack. @maintainers:community @ready_by:Q4 26 @compare:Claude Code, Codex, Cursor Agent
...
15. `compute-supply` Pooled, multi-source compute that the rest of the stack runs on. Right now, we have enough from CSCS and pay for the rest. @maintainers:Finland (LUMI/CSC) + Switzerland (CSCS) + partner institutions @ready_by:Q1 27 @compare:no clean analogue — closed labs own captive datacenters
