			...
			>
				...
				{#if !isLast || !loading}
					<CopyToClipBoardBtn
						...
						classNames="btn inline-flex min-h-7 min-w-7 items-center justify-center rounded-xs p-1 text-sm text-[var(--ap-ink-3)] hover:text-[var(--ap-ink-2)] focus:ring-0"
				...
				{:else}
					...
					>
						...
						<div class="flex w-full flex-row flex-nowrap items-center justify-center gap-2 pt-2">
							...
							<button
								type="button"
								class="btn rounded-xs p-2 text-sm text-[var(--ap-ink-3)] hover:text-[var(--ap-ink-2)] focus:ring-0"
								onclick={() => {
									editMsdgId = null;
								}}
							>
