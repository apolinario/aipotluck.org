...
Projects, repos, and orgs that should be in our data foundation but are missing from both GoodAI List and OSS Insights (coverage/ingestion backlog). This doc is about what to add next; for OSAI “maturity gaps” once data exists, see `currentai.osai_gap_map` and mapping tables in [`models/README.md`](../models/README.md) (analysis guidance lives in persona skills).
...
**Option B — Supplemental static model:** Create `data/catalog-supplements/missing_repos.csv` with the same schema as `goodailist_repos`, upload as a static model, and union it with `ai_repo_activity` in a revised UDM.
