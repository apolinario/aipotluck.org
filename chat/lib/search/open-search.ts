...
// directly over HTTPS from the serverless route (no self-hosted service, no
// closed vendor). Wikipedia (open public knowledge, no key) carries explainers
// and current policy; Marginalia (open independent crawler, public key) adds
// general-web reach at indie scale. A Google-scale open web index does not
// exist — that stays an honest GAP on the map, not something we fake.

import { distillQuery } from "@/lib/search/distill";
...
async function searchMarginalia(
  ...
  const url = `https://api.marginalia.nu/public/search/${encodeURIComponent(
    query
  )}?key=public&count=${limit}`;
