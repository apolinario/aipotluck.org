      ...
      {suggestedActions.map((suggestedAction, index) => (
        ...
        >
          <Suggestion
            ...
            onClick={(suggestion) => {
              const parts = [{ type: "text" as const, text: suggestion }];
              // A recency starter prompt (e.g. the EU AI Act one) goes straight
              // through the open-web-search path — one click demos search + the
              // map flash. runOpenSearch handles the URL update itself.
              if (isRecencyQuery(suggestion)) {
                runOpenSearch({
                  query: suggestion,
                  parts,
                  sendMessage,
